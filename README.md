
<!--<img src="" width="100%">-->
### "I sense a soul in search of answers." [^1]
I’m a software developer with a clinical healthcare background, mostly working with **Python** and the **MERN stack**, building thoughtful systems where software meets real-world complexity.

#### ♡ Beyond code
| 🧙🏻‍♀️ | 🌱 | 🐾 | 🎧 |💻 | 🕹️ | 🎸 |📚|
| - | - | - | - | - | - | - | - |

#### What I’m usually working on
- tools that sit between humans and complex systems  
- automation and data-driven workflows  
- experiments where software replaces unnecessary friction

#### Selected work (production)

- **TrackDataPad** — a small SaaS focused on automating attribution records and dispute workflows for creators 
 ↳ production system · private codebase · https://trackdatapad.com

- **LoopStatic** — a small studio where I design, build, and deploy client-facing systems  
 ↳ private implementations · https://loopstatic.com


#### 💬 You can find me here
[![Twitter Badge](https://img.shields.io/badge/Twitter-FF3366?style=for-the-badge&logo=twitter&logoColor=white)](https://twitter.com/biokoder) [![LinkedIn Badge](https://img.shields.io/badge/LinkedIn-FF3366?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/michelle-c-97a053230/) [![E-mail Badge](https://img.shields.io/badge/biokoder@pm.me-FF3366?style=for-the-badge&logo=@=white)](mailto:biokoder@pm.me) 


[^1]: [Adria the Witch](https://www.youtube.com/watch?v=_z4b4AYQpfc)


